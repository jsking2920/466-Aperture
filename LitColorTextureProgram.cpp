...
Scene::Drawable::Pipeline lit_color_program_pipeline;
...
Load< LitColorTextureProgram > lit_color_program(LoadTagEarly, []() -> LitColorTextureProgram const * {
    LitColorTextureProgram *ret = new LitColorTextureProgram();

    //----- build the pipeline template -----
    lit_color_program_pipeline.program = ret->program_vertex_color;

    lit_color_program_pipeline.OBJECT_TO_CLIP_mat4 = ret->OBJECT_TO_CLIP_mat4;
    lit_color_program_pipeline.OBJECT_TO_LIGHT_mat4x3 = ret->OBJECT_TO_LIGHT_mat4x3;
    lit_color_program_pipeline.NORMAL_TO_LIGHT_mat3 = ret->NORMAL_TO_LIGHT_mat3;

    /* This will be used later if/when we build a light loop into the Scene:
    lit_color_program_pipeline.LIGHT_TYPE_int = ret->LIGHT_TYPE_int;
    lit_color_program_pipeline.LIGHT_LOCATION_vec3 = ret->LIGHT_LOCATION_vec3;
    lit_color_program_pipeline.LIGHT_DIRECTION_vec3 = ret->LIGHT_DIRECTION_vec3;
    lit_color_program_pipeline.LIGHT_ENERGY_vec3 = ret->LIGHT_ENERGY_vec3;
    lit_color_program_pipeline.LIGHT_CUTOFF_float = ret->LIGHT_CUTOFF_float;
    */
    ...
    lit_color_program_pipeline.textures[0].texture = tex;
    lit_color_program_pipeline.textures[0].target = GL_TEXTURE_2D;
...
LitColorTextureProgram::LitColorTextureProgram() {
    ...
    program_vertex_color = gl_compile_program(
            //vertex shader:
            "#version 330\n"
            "uniform mat4 OBJECT_TO_CLIP;\n"
            "uniform mat4x3 OBJECT_TO_LIGHT;\n"
...
LitColorTextureProgram::~LitColorTextureProgram() {
    ...
    glDeleteProgram(program_vertex_color);
	program = 0;
    program_vertex_color = 0;
}
